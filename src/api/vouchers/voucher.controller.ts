...
import { getVouchersByUserId, createOrganizerVoucher } from './voucher.service';
import { z } from 'zod';
...
const createVoucherSchema = z.object({
  ...
  maxDiscount: z.number().positive('Maksimal diskon harus angka positif').optional().nullable(),
...
export const createOrganizerVoucherController = async (req: Request, res: Response) => {
  if (req.user?.role !== UserRole.ORGANIZER) {
    return res.status(403).json({ message: 'Hanya organizer yang bisa membuat voucher.' });
  }
