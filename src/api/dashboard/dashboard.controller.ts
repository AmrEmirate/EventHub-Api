...
import { getOrganizerDashboardData } from './dashboard.service';
...
export const getOrganizerDashboardController = async (req: Request, res: Response) => {
  if (req.user?.role !== UserRole.ORGANIZER) {
    return res.status(403).json({ message: 'Akses ditolak. Hanya untuk organizer.' });
  }
  ...
  try {
    ...
    const dashboardData = await getOrganizerDashboardData(req.user!.id, month, year);

    res.status(200).json(dashboardData);
  } catch (error: any) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ message: "Parameter query tidak valid", errors: error.flatten().fieldErrors });
    }
