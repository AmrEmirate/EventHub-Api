...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (pendingTransactions.length === 0) {
      return res.status(200).json({ message: 'Tidak ada transaksi yang perlu dibatalkan.' });
    }
    ...
    console.log(`Cron job: ${cancelledCount} transaksi yang menunggu konfirmasi telah dibatalkan.`);
    res.status(200).json({ message: `Proses selesai: ${cancelledCount} transaksi berhasil dibatalkan.` });
